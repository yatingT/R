...
```{r}
model = lm(farm~land,area)
plot(x = area$land,y = area$farm)
abline(model$coefficients[1],model$coefficients[2],col = "limegreen")
b = function(X,Y,beta,k){
  n=0
  for(i in 1:length(X)){
    t=Y[i]-beta[1]-beta[2]*X[i]
    if(abs(t)>k){
      t = ((2*k)*abs(t))-k^2
    }else{
      t = (t)^2
...
b1=function(X,Y,beta,k){
  t=c()
  for(i in 1:length(X)){
    temp=Y[i]-beta[1]-beta[2]*X[i]
    if(abs(temp)>k){
      t[i]=2*k*sign(temp)
    }else{
      t[i]=2*(temp)
  ...
  re = c(); re[1] = -mean(t); re[2] = -mean(t*X)
  return(re)
}
opt_nm=optim(par=c(0,0),fn=b,method="Nelder-Mead",X=area$land,Y=area$farm,k=19000)
abline(opt_nm$par[1],opt_nm$par[2],col="navy")

opt_bfgs=optim(par=c(0,0),fn=b,method="BFGS",X=area$land,Y=area$farm,k=19000,gr=b1)
abline(opt_bfgs$par[1],opt_bfgs$par[2],col="black")

opt_cg=optim(par=c(0,0),fn=b,method="CG",X=area$land,Y=area$farm,k=19000,gr=b1)
abline(opt_cg$par[1],opt_cg$par[2],col="coral",lty="dashed")

legend("topright",col =c("limegreen","navy","black","coral"),legend = c("LSE","Nelder-Mead","BFGS","CG"),lty=c(1,1,1,2) )
...
curve(expr = b2,from = -100000,to = 100000)
###because we used x^2 rather than liner, which prevented the curve growth too fast. 
```
...
es=function(Y,beta){
  ...
  pred=c(); pred[1]=Y[1]
  for(i in 2:length(Y)){ 
     form=(beta*Y[i-1])+((1-beta)*pred[i-1]) 
     pred[i]=form
     total=total+(Y[i]-pred[i])^2
  }
  re=total/length(Y)
  return(re)
}
re = optimize(es,interval = c(0,1),Y = nhtemp)
...
```{r}
Y = nhtemp; yhat = c(); yhat[1] = Y[1]
beta = re$minimum
for(i in 2:length(Y)){
  yhat[i] = (beta*Y[i-1])+((1-beta)*yhat[i-1])
}
plot(x = c(1912:1971),y = Y,type = "l",xlab = "Years",ylab = "Tmperature")
lines(x = c(1912:1971),y = yhat,col = "limegreen")
```
...
```{r}
Y = nhtemp;pred1 = c();pred2 = c()
pred1[1] = Y[1];pred2[1] = Y[1]
beta1 = 0.1;beta2 = 0.9
for(i in 2:length(Y)){
  pred1[i] = (beta1*Y[i-1])+((1-beta1)*pred1[i-1])
  pred2[i] = (beta2*Y[i-1])+((1-beta2)*pred2[i-1])
}

plot(x=c(1912:1971),y=Y,type="l",xlab="years",ylab="temperature")
lines(x=c(1912:1971),y=yhat,col="limegreen")
lines(x=c(1912:1971),y=pred1,col="blue")
lines(x=c(1912:1971),y=pred2,col="red")
legend("topright",col=c("black","limegreen","blue","red"), legend=c("original","exponential","beta=0.1","beta=0.9"),lty=c(1,1,1,1))
...
ll=function(p,X){
  n=length(X)
  re = -n*log(1/(sqrt(2*pi)*p[2]))+sum((X-p[1])^2)/(2*p[2]^2)
  return(re)
}
opt3a= optim(p = c(0,1),fn=ll,X = mtcars$mpg)
n = length(mtcars$mpg)
...
sd=sd(mtcars$mpg)*sqrt((n-1)/n)

sample_mean
sd

##my mu is 20.08429, compare to the simple mean is 20.09062
##my sigma is 5.931831 compare to the sd 5.93203
##Both number are relly close
...
```{r}
func3b = function(X,Y,beta){
  re = sum(log(1+exp(beta[1]+beta[2]*X)))-sum(Y*(beta[1]+beta[2]*X))
  return(re)
}
func3b2<-function(x) 1/(1+exp(-(beta1+beta2*x)))
opt3b=optim(par=c(0,0),fn=func3b,X=menarche.cases$age,Y=menarche.cases$reached.menarche)
beta1=opt3b$par[1]; beta2=opt3b$par[2]
beta1
beta2


plot(x=menarche.cases$age,y=jitter(menarche.cases$reached.menarche),xlab="age",ylab="reached.menarche",col='green')
curve(func3b2,from = 9,to = 18,add = T,col = "blue",lwd = 2)
points(x=menarche$Age,y=menarche$Menarche/menarche$Total,pch=2,col="red")
```
